...
<Figure src="../ml-book-assets/Screenshot 2024-11-08 at 7.52.03 PM.png" alt="Decision tree visualization showing prediction path" caption={<><strong>Decision Tree Visualization:</strong> A trained decision tree showing the path from root to leaf for making predictions on the iris dataset.</>} />
...
<Figure src="../ml-book-assets/Screenshot 2024-11-08 at 7.53.20 PM.png" alt="CART algorithm finding best split" caption={<><strong>Finding the Best Split:</strong> The CART algorithm evaluates all possible splits to minimize impurity at each node.</>} />
...
<Figure src="../ml-book-assets/Screenshot 2024-11-08 at 7.50.47 PM.png" alt="Decision tree trained on the iris dataset" caption={<><strong>Iris Dataset Example:</strong> A decision tree trained on the iris dataset using petal length and width as features.</>} />
...
<Figure src="../ml-book-assets/Screenshot 2024-11-08 at 7.54.31 PM.png" alt="Regression tree example" caption={<><strong>Regression Tree:</strong> A decision tree used for regression, where each leaf node predicts the average target value of instances in that region.</>} />
...
<Figure src="../ml-book-assets/Screenshot 2024-11-08 at 7.54.09 PM.png" alt="Overfitting in decision trees" caption={<><strong>Overfitting Example:</strong> A decision tree that has overfit to the training data, creating overly complex decision boundaries.</>} />
...
<Figure src="../ml-book-assets/Screenshot 2024-11-08 at 7.55.03 PM.png" alt="Regularized decision tree with max_depth constraint" caption={<><strong>Regularized Tree:</strong> A decision tree with `max_depth` constraint, showing simpler decision boundaries that generalize better.</>} />
...
<Figure src="../ml-book-assets/Screenshot 2024-11-08 at 7.57.58 PM.png" alt="Effect of regularization hyperparameters" caption={<><strong>Regularization Effects:</strong> Comparison of decision boundaries with different regularization hyperparameter settings.</>} />
...
<Figure src="../ml-book-assets/Screenshot 2024-11-08 at 7.58.22 PM.png" alt="Decision tree sensitivity to data orientation" caption={<><strong>Orientation Sensitivity:</strong> The same dataset rotated by 45 degrees requires a more complex decision boundary with multiple splits.</>} />
...
<Figure src="../ml-book-assets/Screenshot 2024-11-08 at 7.59.01 PM.png" alt="PCA applied to rotated data for decision trees" caption={<><strong>PCA Solution:</strong> Applying PCA to rotated data can help decision trees create more effective splits by aligning the data with the axes.</>} />
...
- **Stochastic Nature of Feature Selection**: When the `max_features` hyperparameter is explicitly set in Scikit-Learn's `DecisionTreeClassifier`, the algorithm randomly selects a subset of features to evaluate at each node. By default, all features are considered at each split with no randomness. However, when `max_features` is set, this randomness introduces another layer of variability as retraining the same decision tree on the same data might result in a different set of features being chosen at each node, leading to a distinct tree structure. To ensure reproducibility, you can set the `random_state` hyperparameter.
